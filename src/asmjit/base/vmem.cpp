...
static const VMemLocal& vMemGet() {
  ...
  if (!vMem.hProcess) {
    ...
    vMem.pageSize = IntUtil::isPowerOf2<uint32_t>(info.dwPageSize);
...
VMemMgr::VMemMgr()
#else
VMemMgr::VMemMgr(HANDLE hProcess) : _hProcess(hProcess)
#endif // ASMJIT_OS_WINDOWS
{
